...
public class HighlightingEffect : MonoBehaviour
{
	...
	void OnPreRender()
	{
		...
		stencilBuffer=RenderTexture.GetTemporary((int)shaderCameraGO.GetComponent<Camera>().pixelWidth,
		(int)shaderCameraGO.GetComponent<Camera>().pixelHeight,stencilZBufferDepth, RenderTextureFormat.ARGB32);
